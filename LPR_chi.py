...
def Image_feature_enhancement(img, debug=False):
    ...
    edged = cv2.Canny(gcikarilmisresim, 40, 200)
...
def License_plate_area(edged, debug=False):
    ...
    if debug:
        i_c = np.copy(img)
        cv2.drawContours(i_c, contours[0], -1, (0, 0, 255), 2)
        plt.imshow(i_c[:, :, ::-1])
        plt.show()
        i_c = np.copy(img)
...
def Affine_transformation(img, pts):
    pts_src = order_points(np.squeeze(pts))
...
def Plate_Recognition(img):
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    edged = Image_feature_enhancement(img,debug=False) # 增强图像特征，最终得到合适的边缘特征
