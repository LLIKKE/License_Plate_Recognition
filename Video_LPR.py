...
if not cap.isOpened():
    ...
    exit()
while True:
    ret,frame = cap.read(120)
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    #filtered = cv2.bilateralFilter(gray, 5, 75,75)
    histogram_e = cv2.equalizeHist(gray)
    ...
    edged = cv2.Canny(gcikarilmisresim, 90, 250)
    ...
    cv2.imshow("binary",binaryerosion)
    ...
    print("detecting the license plate: ", LP)
